...
export const obtenerTodosPorLead  = (id,setData,store) => {
  ...
  export const obtenerTodosPorAsesor  = (leadId,setData,setRefreshing,store) => {
    const { usuario, mostrarNotificacion, mostrarLoader } = store;

 
    let url = ENTRYPOINT+"call/get_by_lead?lead_id="+leadId+"&asesor_id="+JSON.parse(desencriptarJson(usuario)).user.user_ca
    let setting = {
      method: "Get",
      url: url,
      headers: { 'Accept': 'application/json', Authorization: "Bearer " + JSON.parse(desencriptarJson(usuario)).token,  }
  ...
  export const registrarLlamadas = (data, store,toast,setLoading) => {
    const { usuario, mostrarNotificacion, mostrarLoader } = store;
    
    let url = ENTRYPOINT+"call/register";
    let setting = {
      method: "POST",
      url: url,
      data: data,
