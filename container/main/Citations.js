...
export default function Citations({ route, navigation }) {
    ...
    React.useEffect(() => {
        obtenerByDate(date.dateString, lead_id, setData, setRefreshing, initializer)
    }, [])
    function onRefresh() {
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            <TouchableOpacity onPress={() => navigation.navigate('Citation', { id: item.id })} style={{ backgroundColor: '#F8F8F8', padding: 10, borderRadius: 10, marginHorizontal: '5%' }}>
                <Text style={{ fontWeight: 'bold', fontSize: 16 }}>{item.titulo} -  <Text style={{ fontWeight: '100', fontSize: 16 }}>Efectiva: {item.es_efectiva==1?'Sí':'No'}</Text></Text>
                <View style={{ flexDirection: 'row' }}>
                    <View style={{ flex: 1 }}>
                        <Text style={{ color: '#808080' }}>{item.descripcion}</Text>
                    ...
                    <View style={{ backgroundColor: '#323B62', paddingHorizontal: 4, borderRadius: 5 }}>
                        <Text style={{ color: 'white' }}>{new Date(item.fecha).getHours() + ":" + new Date(item.fecha).getMinutes()}</Text>
                    </View>

                </View>
                <Text style={{ marginTop: 3, color: '#808080', fontSize: 15 }}>{item.nombres} {item.apellidos}</Text>
    ...
    const searching = (text) => {
        ...
        let dataB = data.backup.filter(l => {
            return l.titulo.toLowerCase().match(text.toLowerCase());
        });
    ...
    return (
        <View style={styles.container}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 15, alignItems: 'center' }}>
                <Text style={{ fontSize: 24 }}>Lista de citas - {date.dateString}</Text>
                <TouchableOpacity
