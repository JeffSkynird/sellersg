...
import { obtenerDataCliente, editarCliente } from "../../utils/API/clients.js";
...
export default function ClientEdit({ navigation, route }) {
    ...
    useEffect(() => {
        ...
        obtenerDataCliente(null, id, setData)
    }, [])
    useEffect(() => {
        if (data != null) {
            setDni(data.client.celular)
            setNames(data.client.nombres)
            setLastNames(data.client.apellidos)
            setEmail(data.client.email)
            setCellphone(data.client.celular)
            setLandLine(data.client.telefono)
            setAddress(data.client.direccion)
            setCalle2(data.client.calle)
            setDescripcion(data.client.referencia_direccion)
            setDependence(data.client.dependence == 1 ? true : false)
            setVilla(data.client.numero_villa)
            setProvince(data.client.province)
            setCity(data.client.id_ciudad)
            setOtherMonthlyIva(data.client.iva_mensual_otros_ingresos == 1 ? true : false)
            setMonthlyIva(data.client.iva_mensual == 1 ? true : false)
            setStatus(data.client.estado_compra != null ? data.client.estado_compra.toString() : "")
            setRecomendation(data.client.medio_id)
         
            if (data.client.fecha_nacimiento instanceof Date && !isNaN(data.client.fecha_nacimiento)) {
                console.log("Si es")
                setDate(convertirDate(data.client.fecha_nacimiento))
            }
            ...
            setRent(data.client.egreso_renta)
            setClothing(data.client.egreso_ropa)
            setFood(data.client.egreso_comida)
            setEducation(data.client.egreso_educacion)
            setBasic(data.client.egreso_servicios_basicos)
            setTransport(data.client.egreso_transporte)

            setCivil(getId(data.client.id_estado_civil));
            setNamesR(data.client.nombres_referido)
            setDniR(data.client.cedula_referido)
            setProyectoR(data.client.id_proyecto_referido)

            //SPOUSE
            if (data.spouse != null) {
                setDniS(data.spouse.cedula)
                setNamesS(data.spouse.nombres)
                setLastNamesS(data.spouse.apellidos)
                setCellphoneS(data.spouse.celular)
                setEmailS(data.spouse.email)
                setDependenceS(data.spouse.dependencia == 1 ? true : false)
                setLandLineS(data.spouse.telefono)
                setIncomeS(data.spouse.sueldo)
                setOtherIncomeS(data.spouse.sueldo)
                setOtherMonthlyIvaS(data.spouse.iva_mensual_otros_ingresos == 1 ? true : false)
                setMonthlyIvaS(data.spouse.iva_mensual == 1 ? true : false)
            }
        }
    }, [data])
    const getId = (nombre) => {
    ...
    const editar = () => {
        if(validacion()){
            ...
            editarCliente({
                ...
                dependence: dependence == true ? 1 : 0,
                dependenceS: dependenceS == true ? 1 : 0,
                names: names,
            ...
            <ScrollView style={{ flex: 1, height: '100%' }}>
                ...
                <View style={{ marginHorizontal: '5%' }}>
                    ...
                    <Picker
                        ...
                        <Picker.Item label="Estado del prospecto" value="" />
                        <Picker.Item label="Alto (Va a comprar en 8 días)" value="2" />
                        <Picker.Item label="Medio (Va a comprar en un mes)" value="1" />
                        <Picker.Item label="Bajo (No esta decidido)" value="0" />
