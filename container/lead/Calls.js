...
import { obtenerTodosPorAsesor } from '../../utils/API/calls'
...
export default function Citations({ route, navigation }) {
    ...
    React.useEffect(() => {
        obtenerTodosPorAsesor(lead_id, setData, setRefreshing, initializer)
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            <TouchableOpacity style={{ marginBottom: 5, backgroundColor: '#F8F8F8', padding: 10, borderRadius: 10, marginHorizontal: '5%' }}>
                <Text style={{ fontWeight: 'bold', fontSize: 16 }}>{item.names}</Text>
                <View style={{ flexDirection: 'row' }}>
                    <View style={{ flex: 1 }}>
                        <Text style={{ color: '#808080' }}>{item.destination}</Text>
                    ...
                    <View style={{ backgroundColor: '#323B62', paddingHorizontal: 4, borderRadius: 5 }}>
                        <Text style={{ color: 'white' }}>Efectiva: {item.is_effective == 1 ? 'Si' : 'No'}</Text>
                    </View>

                </View>
                <Text style={{ marginTop: 3, color: '#808080', fontSize: 15 }}>{item.created_at}</Text>
    ...
    const searching = (text) => {
        ...
        let dataB = data.backup.filter(l => {
            return l.names.toLowerCase().match(text.toLowerCase());
        });
    ...
    return (
        <View style={styles.container}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 15, alignItems: 'center' }}>
                <Text style={{ fontSize: 24 }}>Historial de llamadas</Text>
                <TouchableOpacity
            ...
            <View style={{ paddingLeft: 15, flexDirection: 'row', marginTop: 10 }}>
                ...
                >
                    <Text style={{ color: selected == '1' ? 'white' : '#323B62', fontWeight: 'bold', textAlign: 'center' }}><Feather name="phone-call" size={18} color={selected == '1' ? 'white' : '#323B62'} /> Salientes</Text>
                </TouchableOpacity>
